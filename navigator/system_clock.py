...
def clock_is_synchronized() -> bool:
    ...
    timex = _Timex()
    state = _adjtimex(ctypes.byref(timex))
    if state < 0:
...
_adjtimex = ctypes.CDLL(None, use_errno=True).adjtimex
_adjtimex.argtypes = (ctypes.POINTER(_Timex),)
_adjtimex.restype = ctypes.c_int
